...
public class cabin
{
    ...
    protected cabin()
    {
        for (int i=0;i<3;i++)
        {
            this.passger_arr[i]=new Passenger();
        }
    }

    // set task3Cabin name,task3Cabin number and task3Cabin size method
    protected void book_cabin(int room_num)
    {
        ...
        while (true)
        {
            System.out.println("Enter how many task3Passenger's like to stay in task3Cabin " + this.cabin_num + "[max=3 passenger] :");
            if (Scan.hasNextInt())
            {
                ...
                {
                    System.out.println("Sorry! Maximum 3 task3Passenger only!!");
                }
            }
            else
            {
    ...
    // getting passengers names to list
